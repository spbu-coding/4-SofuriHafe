...
int get_bmp_info(FILE* file, struct BMP_FILE *bmp_info)
{
    ...
    if(read_uint(&bmp_info->file_size, file) == 0) is_error = true;
    ...
    if(read_uint(&bmp_info->offset, file) == 0) is_error = true;
    ...
    {
        printf("\n8 or 24 bits per pixel are supported\n");
    ...
    if(bmp_info->image_size != 0)
    {
        ...
        {
            printf("\nActual and expected sizes are different\n");

            return -2;
        }
    }

    if(read_uint(&bmp_info->h_resolution, file) == 0) is_error = true;
    if(read_uint(&bmp_info->v_resolution, file) == 0) is_error = true;
...
{
    unsigned int PALETTE_BEGINNING = 54, PALETTE_END = bmp_info->offset;

    unsigned char byte = 0;

    while(ftell(input_file) < bmp_info->file_size)
    {
        ...
        if(bmp_info->bits_per_pixel == 8 && 
            ftell(input_file) > PALETTE_BEGINNING &&ftell(input_file) < PALETTE_END && 
            (ftell(input_file)-PALETTE_BEGINNING)%4 != 0)
        {
            byte = ~byte;
        }

        else if(bmp_info->bits_per_pixel == 24 && ftell(input_file) > bmp_info->offset)
        {
            byte = ~byte;
        }
...
int compare_info(struct BMP_FILE *info1, struct BMP_FILE *info2)
{
    ...
    {
        printf("\nBits per pixel are not similar\n");
    ...
    {
        printf("\nNumber of colors in palette is not similar\n");
...
int compare_pixels(FILE *file1, FILE *file2, struct BMP_FILE *bmp_info, unsigned int *palette_size)
{
    ...
    if(bmp_info->bits_per_pixel == 8)
    {
        ...
        {
            printf("\nUnable to read palette in 1st image\n");
        ...
        {
            printf("\nUnable to read palette in 2nd image\n");
        ...
        for(int i = 0; i < bmp_info->height; i++)
        {
            for(int j = 0; j < bmp_info->width; j++)
            {
                ...
                {
                    printf("\nUnable to read pixels in 1st image\n");
                ...
                {
                    printf("\nUnable to read pixels in 2nd image\n");
                ...
                if(count_of_pixels >= COMPARE_UP_TO) break;
            }
